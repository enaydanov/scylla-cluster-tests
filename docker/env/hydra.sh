...
# failed, a dial-stdio socket keeps retransmitting or has unanswered keepalive probes on either
# end, or sct.py is blocked writing to its stdout. The runner also lists the probe's own
# connection; only sockets whose port the builder holds count.
function hydra_watchdog_anomalies () {
    local rc=$1 builder_view=$2 probe_out=$3 ports stuck='timer:\((on|keepalive),[^,]*,[1-9]'
