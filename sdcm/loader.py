...
class PyCassandraStressHDRExporter(CassandraStressHDRExporter):
    def create_metrix_gauge(self):
        gauge_name = f"collectd_py_cassandra_stress_hdr_{self.stress_operation}_gauge"
        if gauge_name not in self.METRICS_GAUGES:
            self.METRICS_GAUGES[gauge_name] = self.metrics.create_gauge(
                gauge_name,
                "Gauge for py-cassandra-stress hdr percentiles",
                [
                    f"py_cassandra_stress_hdr_{self.stress_operation}",
                    "instance",
                    "loader_idx",
                    "cpu_idx",
                    "type",
...
class PyCassandraStressExporter(StressExporter):
    # Lines containing any of these should be skipped. These are the logs emitted by the `tracing` crate.
    TRACING_LOGS = ["TRACE", "DEBUG", "INFO", "WARN", "ERROR"]
    ...
    def create_metrix_gauge(self):
        gauge_name = f"sct_py_cassandra_stress_{self.stress_operation}_gauge"
        if gauge_name not in self.METRICS_GAUGES:
            self.METRICS_GAUGES[gauge_name] = self.metrics.create_gauge(
                gauge_name,
                "Gauge for py-cassandra-stress metrics",
                [
                    f"py_cassandra_stress_{self.stress_operation}",
                    "instance",
                    "loader_idx",
                    "cpu_idx",
                    "type",
