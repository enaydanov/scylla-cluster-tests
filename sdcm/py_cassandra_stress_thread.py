...
from sdcm.loader import PyCassandraStressExporter, PyCassandraStressHDRExporter
from sdcm.prometheus import nemesis_metrics_obj
from sdcm.provision.helpers.certificate import SCYLLA_SSL_CONF_DIR
from sdcm.reporting.tooling_reporter import PyCassandraStressVersionReporter
from sdcm.sct_events.loaders import PY_CASSANDRA_STRESS_ERROR_EVENTS_PATTERNS, PyCassandraStressEvent
...
class PyCassandraStressEventsPublisher(FileFollowerThread):
    ...
    def run(self) -> None:
        while not self.stopped():
            ...
            for line_number, line in enumerate(self.follow_file(self.log_filename)):
                ...
                for pattern, event in PY_CASSANDRA_STRESS_ERROR_EVENTS_PATTERNS:
                    if self.event_id:
...
class PyCassandraStressThread(CassandraStressThread):
    DOCKER_IMAGE_PARAM_NAME = "stress_image.py-cassandra-stress"
    ...
    def _run_cs_stress(self, loader, loader_idx, cpu_idx, keyspace_idx):  # noqa: PLR0914
        ...
        stress_cmd_opt = self.stress_cmd.split("py-cassandra-stress", 1)[1].split(None, 1)[0]

        log_id = self._build_log_file_id(loader_idx, cpu_idx, keyspace_idx)
        log_file_name = os.path.join(loader.logdir, f"py-cassandra-stress-{stress_cmd_opt}-{log_id}.log")

        LOGGER.debug("py-cassandra-stress local log: %s", log_file_name)
        remote_hdr_file_name = f"hdrh-pycs-{stress_cmd_opt}-{log_id}.hdr"
        LOGGER.debug("py-cassandra-stress remote HDR histogram log file: %s", remote_hdr_file_name)
        local_hdr_file_name = os.path.join(loader.logdir, remote_hdr_file_name)
        LOGGER.debug("py-cassandra-stress HDR local file %s", local_hdr_file_name)
        ...
        try:
            prefix, *_ = stress_cmd.split("py-cassandra-stress", maxsplit=1)
            reporter = PyCassandraStressVersionReporter(argus_client=loader.parent_cluster.test_config.argus_client())
            reporter.report()
        except Exception:  # noqa: BLE001
            LOGGER.info("Failed to collect py-cassandra-stress version information", exc_info=True)
        result = None
        with (
            cleanup_context,
            PyCassandraStressExporter(
            ...
            PyCassandraStressEventsPublisher(node=loader, log_filename=log_file_name) as publisher,
            PyCassandraStressEvent(
                node=loader,
                stress_cmd=self.stress_cmd,
                log_file_name=log_file_name,
            ) as py_cassandra_stress_event,
            PyCassandraStressHDRExporter(
                instance_name=cmd_runner_name,
            ...
            publisher.event_id = py_cassandra_stress_event.event_id
            try:
                # prolong timeout by 5% to avoid killing py-cassandra_stress process
                hard_timeout = self.timeout + int(self.timeout * 0.05)
                with SoftTimeoutContext(timeout=self.timeout, operation="py-cassandra-stress"):
                    result = cmd_runner.run(
                        cmd=node_cmd, timeout=hard_timeout, log_file=log_file_name, retry=0, timestamp_logs=True
                    )
            except Exception as exc:  # noqa: BLE001
                self.configure_event_on_failure(stress_event=py_cassandra_stress_event, exc=exc)

        return loader, result, py_cassandra_stress_event
